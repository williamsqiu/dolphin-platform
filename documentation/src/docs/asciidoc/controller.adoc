...
access the Dolphin Platform from a background thread or endpoint. The event bus is defined by the
 `com.canoo.platform.server.event.DolphinEventBus` interface. Instances are managed objects in the application scope.
...
created by injecting the com.canoo.platform.server.binding.PropertyBinder interface. A more detailed description of
